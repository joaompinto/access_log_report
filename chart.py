...
def build_chart_data(csvfilename, config):
    ...
    time_map = {'hour': 60*60, '10minutes' : 10*60, None : None}
    ...
    with open(csvfilename, 'rb') as csvfile:
        ...
        for row in cvsreader:
            ...
            data_row.append((parser.parse(ts), float(requests) / float(time_interval), int(taken) / int(requests)))
            data[vhost] = data_row

    return data


def create_chart(data, value_id, g_title, y_label, output_filename):

    data_column = COLUMNS_MAP.get(value_id)
    if not data_column:
    ...
    color_i = 0
    for vhost, values in data.iteritems():
        x_values = [i[0] for i in values]
        y_values = [int(i[data_column]) for i in values]
    ...
    for vhost in data.keys():
        plot([], [], color=tableau20[color_i], linewidth=10, label=vhost)
        color_i += 1
    ...
    savefig(output_filename, bbox_inches="tight")
    close()